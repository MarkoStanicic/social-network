...
export class AuthorizeService implements IAuthorizeService {
	...
	public login: (email: string, password: string) => Promise<LoginUser> = (
		...
		return new Promise<LoginUser>((resolve, reject) => {
			axios
				.post(`${config.settings.api}login`, {
					userName: email,
					password: password
				})
				.then(({ data }) => {
				...
				.catch((error: any) => {
					...
					reject(
						new SocialError(
							responseError._code || responseError.code,
							responseError._message || responseError.message
						)
					);
				});
		});
	...
	public getUserRegisterToken = async (user: UserRegisterModel) => {
		try {
			const token = this._httpService.postWithoutAuth(
				`${config.settings.api}user-register-token`,
				{
					user: { ...user }
				}
			);
	...
	public registerUser = async (user: UserRegisterModel) => {
		try {
			const signupResult = await firebaseAuth().createUserWithEmailAndPassword(
				user.email as string,
				user.password as string
			);

			const { uid, email } = signupResult.user!;
			const result = await this.storeUserInformation(
				uid,
				email!,
				user.fullName,
				"",
	...
	) => Promise<void> = (newPassword, confirmPassword) => {
		return new Promise<void>((resolve, reject) => {
			...
			if (user) {
				firebaseAuth()
					...
					.then(tokenId => {
						axios
							.post(
								`${config.settings.api}update-password`,
								{
									newPassword,
									confirmPassword
								},
							...
							.catch(error =>
								reject(
									new SocialError(error.code, `authorizeSerive/UpdatePassword`)
								)
							);
					})
					.catch((error: any) =>
						reject(new SocialError(error.code, `authorizeSerive/TokenError`))
					);
			}
		});
	};
		...
		if (currentUser) {
			...
			try {
				const result = await axios.post(
					`${config.settings.api}email-verification`,
					{
				...
				throw new SocialError(
					error.code,
					`authorizeSerive/sendEmailVerification`
				);
			}
		}
	};
			...
			if (currentUser) {
				currentUser
					...
					.then(tokenId => {
						axios
							.post(
								`${config.settings.api}sms-verification`,
								{
									phoneNumber,
									"g-recaptcha-response": value
								},
	...
	public sendResetPasswordVerification = (email: string, value: any) => {
		...
		return new Promise<string>((resolve, reject) => {
			axios
				.post(`${config.settings.api}email-verification-code`, {
					email,
					"g-recaptcha-response": value
				})
				.then(({ data }) => {
		...
		return new Promise<any>((resolve, reject) => {
			firebaseAuth()
				...
				.then(tokenId => {
					axios
						.post(
							`${config.settings.api}verify-phone`,
							{
								code,
								verifyId,
								phoneNumber
				...
				.catch((error: any) =>
					reject(
						new SocialError(
							error.code,
							`authorizeSerive/confirmVerificationPhone  ${error.message}`
						)
					)
				);
		});
	...
	public confirmVerificationEmail = async (code: string, verifyId: string) => {
		try {
			...
			const result = await axios.post(
				`${config.settings.api}verify-email`,
				{
					code,
					verifyId
				},
			...
			throw new SocialError(
				error.code,
				`authorizeSerive/confirmVerificationEmail  ${error.message}`
			);
		}
	};
	...
	public confirmResetPassword = (
		...
		return new Promise<any>((resolve, reject) => {
			axios
				.post(`${config.settings.api}verify-reset-password`, {
					code,
					verifyId,
					email
				})
				...
				.catch((error: any) =>
					reject(
						new SocialError(
							error.code,
							`authorizeSerive/confirmVerificationCode`
						)
					)
				);
		});
