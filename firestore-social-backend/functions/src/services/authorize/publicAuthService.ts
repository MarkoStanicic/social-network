		...
		.then(result => {
			...
			if (result && !result.empty && result.size === 1) {
				...
				return res
					.status(HttpStatusCode.InternalServerError)
					.send(
						new SocialError("ServerError/WrongUserName", "User name is wrong!")
					);
			}
		})
		.catch(error => {
			return res
				.status(HttpStatusCode.InternalServerError)
				.send(new SocialError("ServerError/FirestoreGetData", error));
		});
});
		...
		.then(userInfo => {
			...
			if (userInfo && !userInfo.empty && userInfo.size === 1) {
							...
							if (Number(verification.code) === Number(code)) {
								firestoreDB
									...
									.then(protectedUserResult => {
										...
										if (
											protectedUserResult.exists &&
											protectedUserResult.data().phoneVerified
										) {
											phoneVerified = true;
										}
										console.log(
											"ServerSuccess/CodeAccepted",
											"CodeAccepted",
											phoneVerified
										);
										const additionalClaims = {
										...
										adminDB
											...
											.catch(error => {
												...
												res
													...
													.json(
														new SocialError(
															"ServerError/CreateCustomToken",
															"Create custom token error!"
														)
													);
											});
									});
				...
				res
					...
					.send(
						new SocialError("ServerError/EmailNotFound", "Email not found!")
					);
			}
		})
		.catch(error => {
			res
				.status(HttpStatusCode.NotFound)
				.send(new SocialError("ServerError/EmailNotFound", "Email not found!"));
		});
});
	...
	firestoreDB
		...
		.then(userInfoList => {
			if (userInfoList.size === 1) {
				...
				request(verificationURL, (error: any, response: any, body: any) => {
					...
					emailAPI
						.sendEmail(new Email(from, to, subject, html))
						...
						.catch(error => {
							res
								...
								.send(
									new SocialError(
										"ServerError/EmailNotSent",
										"Email service error. Email has not sent!"
									)
								);
						});
				});
		...
		.catch(error => {
			res
				...
				.send(
					new SocialError(
						"ServerError/EmailNotFound",
						"With DB error. Email not found!"
					)
				);
		});
});
