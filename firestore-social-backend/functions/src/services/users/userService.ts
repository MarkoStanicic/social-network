 ...
 */
export const onUpdateUserInfo = functions.firestore
	...
	.onUpdate((dataSnapshot, context) => {
		return new Promise<void>((resolve, reject) => {
			...
			if (
				previousUserInfo.avatar === userInfo.avatar &&
				previousUserInfo.fullName === userInfo.fullName
			) {
				resolve();
			} else {
				const postsRef = firestoreDB
					.collection("posts")
					.where("ownerUserId", "==", userId);
				const commentsRef = firestoreDB
					.collection("comments")
					.where("userId", "==", userId);
				const leftUserTieRef = firestoreDB
					.collection("graphs:users")
					.where("leftNode", "==", userId);
				const rightUserTieRef = firestoreDB
					.collection("graphs:users")
